...
class RobotSimEnv(gym.Env):
    ...
    def reset(self, seed=23,randomize = True):
        ...
        while not selectedPath:
            self.random_idx = np.random.choice(self.random_idx_list)
            #self.random_idx = 47 # comment for actual training
            self.realPath = np.load(f"real_spline_attackPaths/path_{self.random_idx}.npy")
            self.realPath = self.realPath[:,0,:]
            if self.realPath.shape[0] > 1:
                selectedPath = True
            else:
                self.random_idx_list.remove(self.random_idx)
 
        self.hitting_point = self.realPath[-1,:]
        self.pathlen = self.realPath.shape[0]
        self.currentpathlen = self.pathlen
        self.achieved_step = 0
        path = np.load(f"attackPaths/path_{self.random_idx}.npy")
